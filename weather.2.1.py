...
def get_geoloc_list(file_name):
	df = pd.read_excel(file_name, sheet_name = 0)
	geoloc_list = df['CompanyNumber'].tolist()
	return geoloc_list

def get_forecast(geoloc_list, coordinates_list): #takes a list of coordinates and returns the forecast
	#initiantes lists
	dates = []
	rain_amounts = []
	coordinates = []
...
list_of_geolocs = get_geoloc_list(excel_file)
list_of_coords = coord_list(excel_file)
forecast = get_forecast(list_of_geolocs, list_of_coords)
